...
class ClienteEcommerceController extends Controller
{
	...
	{
		$this->_validate($request, true, $id);
	...
	private function _validate(Request $request, $edit = false, $id = null){
		$rules = [
			'nome' => 'required|max:30',
			'sobre_nome' => 'required|max:30',
			'email' => ['required', 'email', 'max:60', \Illuminate\Validation\Rule::unique('cliente_ecommerces')->ignore($id)],
			'cpf' => ['required', 'max:18', new ValidaDocumento, \Illuminate\Validation\Rule::unique('cliente_ecommerces')->ignore($id)],
