...
class SuperadminSubscriptionsController extends BaseController
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            ->addColumn(
                'action',
                '<button data-href ="{{action(\'\Modules\Superadmin\Http\Controllers\SuperadminSubscriptionsController@edit\',[$id])}}" class="sa-btn-pill sa-btn-pill-outline change_status" data-toggle="modal" data-target="#statusModal">
                @lang( "superadmin::lang.status")
                </button> <button data-href ="{{action(\'\Modules\Superadmin\Http\Controllers\SuperadminSubscriptionsController@editSubscription\',["id" => $id])}}" class="sa-btn-pill sa-btn-pill-primary btn-modal" data-container=".view_modal">
                @lang( "messages.edit")
                </button>'
            )
            ...
            ->editColumn(
                'status',
                '@if($status == "approved")
                <span class="sa-badge-pill sa-badge-green">{{__(\'superadmin::lang.\'.$status)}}
                </span>
                @elseif($status == "waiting")
                <span class="sa-badge-pill sa-badge-blue">{{__(\'superadmin::lang.\'.$status)}}
                </span>
                @else($status == "declined")
                <span class="sa-badge-pill sa-badge-red">{{__(\'superadmin::lang.\'.$status)}}
                </span>
                @endif'
            )
            ->editColumn(
